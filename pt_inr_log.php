<?php
	...
	$sql	=	"select * from pt_inr where patient_id='".$bc_patient_id."' $query_filter_left $query_filter_top";
	$res	=	mysql_query($sql);
?>
<style type="text/css">
.whiteMiddle .evField {
    ...
    <table width="100%" cellpadding="2" cellspacing="0" border="0">
           ...
           <?php if(mysql_num_rows($res)>0){
			...
			while ($row = mysql_fetch_assoc($res) ) {
				...
				$ptinr_min_range		=	$row["ptinr_min_range"];
				$ptinr_max_range		=	$row["ptinr_max_range"];
				
				//Reading
				if($bc_reading > $ptinr_min_range)
					$bc_pt_inr = '<b style="color:#D05600">'.$bc_reading.'</b>&nbsp;<img src="'.ABSOLUTE_PATH.'images/icon/fever_up.png"/>';
				elseif($bc_reading < $ptinr_max_range)
			...
			?>
              <tr  class="log_details <?php if($no&1){?>bk2<?php }else{?>bk1<?php }?>" onmouseover="showDisplay(<?php echo $row['id'];?>);" id="overColor<?php echo $row['id'];?>" onmouseout="hideThis(this.id);">
                ...
                <td>
                   ...
                   <a id="editR<?php echo $row['id'];?>" class="hideAll" href="pt_inr.php?id=<?php echo $row['id'];?>"><img src="<?php echo ABSOLUTE_PATH;?>images/icon/edit_record.png" title="Edit"/></a>
                 </td>
         	</tr>
            <?php $no++;
			} }else{?>
...
function filterLeft(id){
	<?php if(isset($_GET['filter_top']) && $_GET['filter_top']!=''){?>
	 window.location.href = '<?php echo ABSOLUTE_PATH;?>pt_inr.php?p=log&filter_left='+id+"&filter_top=<?php echo $_GET['filter_top'];?>";
	 <?php }else{?>
	 	 window.location.href = '<?php echo ABSOLUTE_PATH;?>pt_inr.php?p=log&filter_left='+id;
	 <?php }?>
	}
function filterTop(id){
	<?php if(isset($_GET['filter_left']) && $_GET['filter_left']!=''){?>
	 window.location.href = '<?php echo ABSOLUTE_PATH;?>pt_inr.php?p=log&filter_top='+id+"&filter_left=<?php echo $_GET['filter_left'];?>";
	 <?php }else{?>
	 	 window.location.href = '<?php echo ABSOLUTE_PATH;?>pt_inr.php?p=log&filter_top='+id;
	 <?php }?>
	}	
</script>
<script type="text/javascript">
						  ...
						  $.ajax({
							...
							success: function(data){
								if(data=='1'){
									...
									alert("Some Error Occure Tyr Again !")
								}
							}
							});
						},
