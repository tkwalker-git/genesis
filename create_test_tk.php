...
$bc_cost			=	$_POST["cost"];
$bc_retail_cost		=	$_POST["retail_cost"];
...
if (isset($_POST["submit"]) ) {
	...
	if (!count($errors)) {
		 ...
		 if ($action1 == "save") {
			$sql	=	"insert into tests (test_name,description,suppliers,cost,retail_cost,clinic_id) values ('" . $bc_test_name . "','" . $bc_description . "','" . $bc_suppliers . "','" . $bc_cost . "','" . $bc_retail_cost . "','" . $_SESSION['LOGGEDIN_MEMBER_ID'] . "')";
			$res	=	mysql_query($sql);
		...
		if ($action1 == "edit") {
			$sql	=	"update tests set test_name = '" . $bc_test_name . "', description = '" . $bc_description . "', suppliers = '" . $bc_suppliers . "', cost = '" . $bc_cost . "', retail_cost = '" . $bc_retail_cost . "' where id=$frmID";
			$res	=	mysql_query($sql);
			if ($res) {
...
if ($res) {
	if ($row = mysql_fetch_assoc($res) ) {
		...
		$bc_cost			=	$row["cost"];
		$bc_retail_cost		=	$row["retail_cost"];
	} // end if row
	$action = "edit";
} // end if 
            ...
            <div class="whiteMiddle" style="padding-top:1px;">
              <div id="accordion">
                ...
                <div style="float:left; width:33%">
                  	...
                  	<input type="text" name="retail_cost" id="retail_cost" class="new_input" value="<?php echo $bc_retail_cost; ?>"/>
                  </div>
