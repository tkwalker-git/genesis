...
if ( isset($_POST['continue']) )
{
	...
	if(!isset($_POST['subscription_type']))
		$errors[] = "Select Subscription type!";
		...
		try
		{
			...
			if ($subscription->isSuccessful())
			{
				...
				if($_POST["action"] == 0){
					...
					$sql = "update patients set `auth_subscription_id`='$subscription_id' where id='". $member_id ."'";
					$res = mysql_query($sql);	
				...
				if($res){
					if($_POST["action"] == 0){
						$err = "Subscruption Created successfully";
					}else{
						$err = "Subscruption Updated successfully";	
					}
...
<div class="yellow_bar"> &nbsp; <?php if($subscription_type > 0){ ?>UPDATE YOUR SUBSCRIPTION<?php }else{ ?>CREATE YOUR SUBSCRIPTION<?php } ?></div>
<!-- /yellow_bar -->
<div style="padding:0 10px;"><br />
<div class="error" style="color:red; font-size:18px; text-align:center;"><?php echo $err; ?></div>
	

<div class="ew-heading">Subscriptions <?php if($subscription_type > 0){ ?> <a href="?p=patient-profile&cancel_subscription=1">Cancel Subscriptions</a><?php } ?></div>

	<form action="" method="post" name='profrm' enctype="multipart/form-data">
	<input type="hidden" name="action" value="<?php echo $subscription_type ?>"  />
      <div class="clr"></div>
    <div class="editProox">
    
    	<?php if($subscription_type == 0){ ?> 
        	<div style="font-weight:bold; color:red; text-align:center;">
            	You are not subscribed yet please fill the form below for creating subscription.
            </div>
		<?php } ?>
