...
if(tableEventExists("BeforeProcessReport",$strTableName)) 
{
...
elseif(is_wr_custom())
{
	if(GetDatabaseType()!=1)
	{
		...
		$strSQL = "select * from (" .$sqlquery .") as t " . $rpt_array['where'] . $rpt_array['order_by'];
	}
	else
...
$xt->assign("search_type", (!is_wr_project()) ? "dsearch" : htmlspecialchars($rpt_array['short_table_name'])."_search");
$xt->assign( "ext", "php" );
...
if(!$editmode)
{
	if(tableEventExists("BeforeShowReport",$strTableName))
	{
		$eventObj =&getEventObject($strTableName);
		$eventObj->BeforeShowReport($xt,$templatefile);
	}
