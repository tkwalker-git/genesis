	...
	function collect_input_data() {
		...
		$("#ugroups").children().each(function(i){
			output.permissions[i] = {
				...
				view : $("#view_prm_"+this.value).attr("checked").toString(),
				edit : $("#edit_prm_"+this.value).attr("checked").toString()
			};
		});
...
foreach ( $arrUserGroups as $arr ) {
	$userGroups .= '<option value="'.$arr[0].'">'.htmlspecialchars($arr[1]).'</option>';
