...
if(is_wr_project())
{
	include("include/" . GetTableURL($root['tables'][0]) . "_settings.php");	
}
...
{
		$dataSeries .= '<option value="'.htmlspecialchars($arr["field"]).'">'.htmlspecialchars($arr["field"]).'</option>';
...
{
		$strLabel .= '<option value="'.htmlspecialchars($arr["field"]).'">'.htmlspecialchars($arr["field"]).'</option>';
}
/*
foreach ($arr_label_series as $arr) {
...
$(document).ready(function(){
	...
	function add_dataseries(num)
	{
		...
		dataseries.find("td:eq(0)").html("Data Series "+num);
		';
if ($_SESSION['webcharts']['chart_type']['type']!="ohlc" && $_SESSION['webcharts']['chart_type']['type']!="candlestick")
{
	$b_includes .= '	
			dataseries.find("td:eq(2)").attr("id","label_title_"+num);
			dataseries.find("td:eq(3)").attr("id","label_value_"+num);		
	...
	{
	$b_includes .= '
			dataseries.find("td:eq(4)").attr("id","picker_div"+num);
			dataseries.find("td:eq(5)").attr("id","picker_selector"+num);
	...
	$b_includes .= '	
			...
			sizeseries.attr("id","");
			sizeseries.find("td:eq(0)").html("Size Series "+num);
...
else
{
	$b_includes .= '
			...
			dataseries3.find("td:eq(0)").attr("id","label_title_"+num);
			dataseries3.find("td:eq(1)").attr("id","label_value_"+num);	
			
			dataseries3.find("td:eq(3)").attr("id","picker1_div"+num);
			dataseries3.find("td:eq(4)").attr("id","picker1_selector"+num);
			dataseries3.find("td:eq(5)").attr("id","picker2_div"+num);
			dataseries3.find("td:eq(6)").attr("id","picker2_selector"+num);
...
$b_includes .= '
	...
	$("td[id^=picker]").hide();
	$("div[id^=picker]").css("background-color", "#FF0000");
...
$b_includes .= '
		if ( value != "" && value != null)
		{
			...
			$(tr).find("td[id^=picker_selector]").show();
...
$b_includes .= '
			...
			$(tr).find("input[id^=series_label]").val("");'."\r\n";
...
if ( !empty( $arr ) ){
	for ( $i=0; $i < count($arr)-1; $i++ ) 
	{
		...
		$b_includes .= '
			...
			$("#picker' . ($i+1) . '").css("background-color", "#'.$arr[$i]["series_color"].'");'."\r\n";
		if ($_SESSION['webcharts']['chart_type']['type']=="ohlc" || $_SESSION['webcharts']['chart_type']['type']=="candlestick")
		{
