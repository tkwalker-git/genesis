...
for ($i=0; $i < count($arr_join_tables); $i++ ) {
	...
	for ($j=0; $j < count($arr_fields); $j++) 
	{
		...
		$fields .= "<option value=\"".htmlspecialchars($t.".".$field)."\">".$t.".".$field."</option>";
	}
}
...
';
	...
	function collect_input_data() {
		...
		$(".condition-row").each(function(){
			if ($(this).find("td.field_value select").val() != "-1" && $(this).find("td.filter_value input").val() != "") 
			{
...
*/
		...
		if( $(this).attr("id").substr(0,3)=="row" && $(this).attr("id") != "row2" )
			URL = "webreport" + $(this).attr("id").replace("row", "") + ".php";
