...
if ($_REQUEST['signed_request']) {
  $response = parse_signed_request($_REQUEST['signed_request'],FACEBOOK_SECRET2);
	...
	if ( isset($response['registration'])) {
			...
			$username 		= DBin($response['registration']['email']);
		...
		$userChk_q   = "select username, email from users where username = '$username'";
		$Chk_email_q = "select email from users where email = '$email'";
		
		$user_res = mysql_query($userChk_q);
		if($user_r = mysql_fetch_assoc($user_res)){
				$userDb = trim($user_r['username']);
		}
		...
		if ( count( $errors) > 0 ) {
			...
			$name = $fname;
			$user_insert = "insert into users (firstname,lastname, email, username, password,zip,city,sex,dob,facebookid, varifaction_code,createddate, usertype,email_verify,enabled) 
							values ('$name','$lname', '$email', '$username', '$password',$zip,$city,$gender,$dob,$facebookid, '$varifactionCode', '$memberdate', '$user_type','1','1') ";
			//d($user_insert,1);
...
//	$sql_ref = "insert into member_referals (ref_member_id,member_id) VALUES ('". $ref_member ."','". $_SESSION['insertid'] ."')";
				...
				if($usertype == "p")
					echo "<script>window.location.href='signup-promoter2.php';</script>";
				else
	...
	if ( isset($_POST['continue1']) || isset($_POST['continue1_x']) ) {
		...
		if ( count( $errors) > 0 ) {
			...
			$name = $fname;
			$user_insert = "insert into users (firstname,lastname, email, username, password, varifaction_code,createddate, usertype,email_verify,enabled) 
							values ('$name','$lname', '$email', '$username', '$password', '$varifactionCode', '$memberdate', '$user_type','1','1') ";
       ...
       <form action="" method="post" name="signupform" id="signupform" enctype="multipart/form-data">
								...
								<div class="input-row">
									<div class="input-col-l" style="width:100%;">
										<div class="input-field" style="width:25px;" id="ut"><input type="checkbox"  name="usertype" value="p" / style="width:20px; height:20px;" ></div>
									...
									<div class="input-row">
										...
										<div class="input-field">
											<input type="text" class="required" name="username" id="username" onblur="this.value=removeSpaces(this.value);" value="<?php if(isset($_POST['username']) && $_POST['username'] != '') echo html_entity_decode($_POST['username'],ENT_QUOTES);?>" maxlength="20" />
										</div>
									</div><!-- input-row -->
									...
									<div class="input-row">
										...
										<div class="input-field">
											<input type="password" class="required" name="password" id="password" onblur="this.value=removeSpaces(this.value);" maxlength="10"  />
										</div>
									</div><!-- input-row -->
									...
									<div class="input-row">
										...
										<div class="input-field">
											<input type="password" class="required" name="cpassword" id="cpassword" onblur="this.value=removeSpaces(this.value);" maxlength="10"  />
										</div>
									</div><!-- input-row -->
