...
function getChargifyProducts() 
{
	...
	curl_setopt($ch, CURLOPT_URL, "https://big-chillin.chargify.com/products.xml");
	...
	if ( $result->code > 0 ) {
		...
		$productsArr	= $data['PRODUCTS'][0]['PRODUCT'];
		
		$products 		= array();
		
		foreach ( $productsArr as $product ) {			
			$price = ((int)$product['PRICE_IN_CENTS'][0]['VALUE'])/100;
			
			// array ( NAME, PRICE, DESCRIPTION, TRIAL_INTERVAL , TRIAL_INTERVAL_UNIT, TRIAL_PRICE_IN_CENTS )
   $products[$product['ID'][0]['VALUE']] = array ($product['NAME'][0]['VALUE'],$price,$product['DESCRIPTION'][0]['VALUE'] ,$product['TRIAL_INTERVAL'][0]['VALUE'],		$product['TRIAL_INTERVAL_UNIT'][0]['VALUE'],$product['TRIAL_PRICE_IN_CENTS'][0]['VALUE']);
   
			// array ( NAME, PRICE, DESCRIPTION )
			//$products[$product['ID'][0]['VALUE']] = array ($product['NAME'][0]['VALUE'],$price,$product['DESCRIPTION'][0]['VALUE']);
		}
...
function logo(){
	...
	$id=$_SESSION['LOGGEDIN_MEMBER_ID'];
if($_SESSION['LOGGEDIN_MEMBER_ID']){
	if($type=='patient'){
		$cid = getSingleColumn("clinicid","select * from `patients` where `id`='".$id."'");
		$img = getSingleColumn("logo_img","select * from `users` where `id`='".$cid."'");
	}
	if($type=='doctor' || $type=='clinic' ){
		$img = getSingleColumn("logo_img","select * from `users` where `id`='".$id."'");
	}
if($img)
	$logo = "images/logos/".$img;
else 
	$logo = "images/Default-logo.png";
	
}
else {
$logo = "images/restoration-health-logo2.png";
}
return $logo;
}

function invitep_logo($did){
$img = getSingleColumn("logo_img","select * from `users` where `id`='".$did."'");
if($img){
	$logo = "images/logos/".$img;
	}
else {
...
$rest	=	mysql_query("select * from `affiliate` where `clinic_id`='".$_SESSION['LOGGEDIN_MEMBER_ID']."'");
$have_rest	=	mysql_num_rows($rest);
if($have_rest){
$aid  =	getSingleColumn("affiliate_id","select * from `affiliate` where `clinic_id`='".$_SESSION['LOGGEDIN_MEMBER_ID']."'");
}
else {
$aid = "";
}
...
function Complete_mnoviforms_response(){
$Pat_id=$_SESSION['LOGGEDIN_MEMBER_ID'];
$get_mnoviforms_response=mysql_query("select * from `mnoviforms_response` where `Patient Id`='$Pat_id'");
$have_mnoviforms_response=mysql_num_rows($get_mnoviforms_response);
if($have_mnoviforms_response){
	$complete=33;	
}
else {
$complete=0;
...
function Complete_request_appt(){
$Pat_id=$_SESSION['LOGGEDIN_MEMBER_ID'];
$doc_id=getSingleColumn("clinicid","select * from `patients` where `id`='$Pat_id'");
$get_request_appt=mysql_query("select * from `request_appt` where `patient_id`='$Pat_id' && `clinic_id`='$doc_id'");
$have_request_appt=mysql_num_rows($get_request_appt);
if($have_request_appt){
	$complete=33;	
}
else {
$complete=0;
...
function patient_status($val){
if($val==1){
$p_stat="Request";
}elseif($val==2){
$p_stat="Scheduled";
}elseif($val==3){
$p_stat="Review Required";
}elseif($val==4){
$p_stat="Released";
}elseif($val==5){
$p_stat="No Show";
...
function patient_status_color($val){
if($val==1){
$p_colors="#0000ff";
}elseif($val==2){
$p_colors="#7f007f";
...
$Pat_id=$_SESSION['LOGGEDIN_MEMBER_ID'];
$doc_id=getSingleColumn("clinicid","select * from `patients` where `id`='$Pat_id'");
 $get_plan=mysql_query("select * from `plan` where `patient_id`='$Pat_id' && `clinic_id`='$doc_id'");
$have_get_plan=mysql_num_rows($get_plan);
if($have_get_plan){
	$complete=33;	
}
else {
...
function final_progress(){
$final_prog = Complete_mnoviforms_response()+Complete_request_appt()+Complete_plan();
return $final_prog;
}

function validateID($userid,$tbl,$id){
if($tbl=='patients' || $tbl=='users'){
$sql_chk=mysql_query("select * from ".$tbl." where clinicid='".$userid."' and id='".$id."'");
}
elseif($tbl=='protocols' || $tbl=='tests' || $tbl=='learning_library' || $tbl=='plan' || $tbl=='supplement' || $tbl=='findings'){
$sql_chk=mysql_query("select * from ".$tbl." where clinic_id='".$userid."' and id='".$id."'");
}
else {
$sql_chk=mysql_query("select * from ".$tbl." where clinic_id='".$userid."' and patient_id='".$id."'");
}
if(mysql_num_rows($sql_chk)){
 $return="true";
}
else {
 $return="false";
...
function validateEID($userid,$tbl,$id,$pid){

if($id){
	
	 $sql_chk=mysql_query("select * from ".$tbl." where userid='".$userid."' and id='".$id."'");
}
elseif($pid){

	$sql_chk=mysql_query("select * from ".$tbl." where userid='".$userid."' and patient_id='".$pid."'");
}
...
try {
			...
			//var_dump($results);
			 ...
			 $reference = $results->return->ClinicID;
...
{
//	if(getSingleColumn("event_type","select * from `events` where `id`='$event_id'")!=0){
...
//	$sql = "select seo_name from categories order by id LIMIT 1";
	...
	return ABSOLUTE_PATH_WITHOUT_SSL . 'category/' . $seo_name . '.html';
}
...
function userSubMenu2($selected){
	...
	$pages = array(
				...
				ABSOLUTE_PATH."fbapp/facebook_flyer_settings" => "Facebook",
	...
	foreach ($pages as $name => $title) {
		...
		if 	( $k == count($pages) )
			...
			$stl = '';	
?>
<li <?php echo $stl;?>><a href="<?php echo $name;?>.php" <?php echo $cls;?> id="a5" ><strong><?php echo $title;?></strong></a></li>
<?php
	}
 ...
 function dashboardTab($selected){
 	$pages = array(
				"dashboard" => "Dashboard",
				"fbapp/remove_tab" => "Promote Your Event",
				"create_event" => "Create Event",
				"myeventwall" => "My Eventwall",
				"manage_event" => "Event Manager"
				//"profile_setting" => "My Profile"
				//"index" => "Settings"
				);
	
	echo '<ul class="dashbdTabs">';			
	$k=0;
	foreach ($pages as $name => $title) {
		$k++;
		if ( $name == $selected )
...
function makeRecommendedQueryExtended($category="",$sele) {
	...
	$sql = "select * from events where event_status='1' ".$userZip."". $catFilter ." AND is_expiring=1 AND 
			( 	
			...
			select * from events where event_status='1' ".$userZip."". $catFilter ." AND is_expiring=1 AND
			( 	
				subcategory_id IN (" . $subCat . ")
				AND
				id IN (select event_id from event_music where music_id IN (". $musGen .") )
...
$sql = "select * from events where featured='1' AND event_status='1' ".$userZip." ". $catFilter ." AND is_expiring=1 AND
...
function checkValidZip($zip){
	$res = mysql_query("select * from `zipcodes` where `zipcode`='".$zip."'");
...
function calculateDiscount($event_id,$ccode,$amount)
{
	if($event_id=='create')
		$ad = " and created_by=0";

	$sql ="select * FROM coupons where code = '$ccode' and active='yes'".$ad."";
	...
	if ( $result ) {	
		...
		if($row['created_by']!=0)
			$event_userid	= getSingleColumn('userid',"select * from `events` where id=$event_id");
		else
			$event_userid	= 0;
		
		if($row['created_by']==$event_userid || $event_id=='create'){
			...
			if ($row['coupon_type'] == 3) {
				...
				$limit = $row['use_limit'];
				$rs = mysql_query("select * from orders where coupon_code='". $ccode ."' && `total_price`!=''");
...
function generateTicketsPDF($order_id) {
	...
	while ( $rows1 = mysql_fetch_assoc($res1) ) {
		...
		$order_type			= attribValue('orders', 'type', "where id='$order_id'");
		$table_number		= attribValue('orders', 'main_ticket_id', "where id='$order_id'");
          ...
          <td width="390" align="right" valign="top"><table width="95%" border="0" cellspacing="0" cellpadding="0" align="right">
              ...
              <tr>
                <td ><table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					if($order_type=='table'){?>
					<tr>
                      <td class="labels">Table#:</td>
                      <td class="labels"><?php echo $table_number;?></td>
                    </tr>
					<?php
					}
					else{?>
...
<table width="840" border="0" cellspacing="0" cellpadding="0" align="center" >
  ...
  <tr>
    <td><img src="<?php echo IMAGE_PATH; ?>ticket_cutter.jpg" border="0" /></td>
  </tr>
</table>
<?php
$html = ob_get_contents();
...
function getDays($specials_id){
	...
	while($rows = mysql_fetch_array($ress)){
		...
		$ev_id	= getSingleColumn('id',"select * from `events` where id=$t_event_id");
...
function emailAttachment($to,$file,$subject,$message,$fileType,$filename){
		
		$strresume = DOC_ROOT.$file;
		$strresume_size=filesize($strresume);
		...
		//Normal headers
		$headers  = "From: Your Health Suppor <info@yourhealthsupport.com>\r\n";
...
function getTicketsRecord($event_id,$type){
	$ticket_id = getSingleColumn('id',"SELECT * FROM `event_ticket` where `event_id`='$event_id' && `event_id`!=''");
	if($ticket_id>0){
		$res = mysql_query("select * from `orders` where `main_ticket_id`='$event_id' && `type`='ticket' && `total_price`!=0");
		$sold_tickets = 0;
		while($row = mysql_fetch_array($res)){
			$gross_sales = $row['total_price']+$gross_sales;
			$order_id = $row['id'];
			$res2 = mysql_query("select * from `order_tickets` where `order_id`='$order_id'");
			$sold_tickets = mysql_num_rows($res2)+$sold_tickets;
		}
		
		$total_tickets = getSingleColumn('quantity_available',"select * from `event_ticket` where `event_id`='$event_id'");
		
		if($type == 'Sold')
			return $sold_tickets;
		elseif($type == 'Gross Sales')
			return number_format($gross_sales, 2);
		elseif($type == 'Tickets Left')
...
function updateView($event_id){
	if($_SESSION['view'][$event_id]!='yes'){
		$view = getSingleColumn('view',"select * from `events` where `id`='$event_id'");
		$add = $view+1;	
		$res = mysql_query("UPDATE `events` SET `view` = '$add' WHERE `id` = '$event_id'");
		if($res)
		$_SESSION['view'][$event_id] = 'yes';
	}
}

function conversionRate($event_id){
	if($event_id){
		$view = getSingleColumn('view',"select * from `events` where `id`='$event_id'");
		$sold = getTicketsRecord($event_id,'Sold');
		return number_format($sold*100/$view);
	}
	else
		return 0;
}
...
function timeTillEvent($event_id){

	$eventStartDate		= getSingleColumn('event_date',"select * from `event_dates` where `event_id`='$event_id' ORDER BY `event_date` ASC LIMIT 0,1");
	$eventStartDateId	= getSingleColumn('id',"select * from `event_dates` where `event_id`='$event_id' ORDER BY `event_date` ASC LIMIT 0,1");
	$eventStartTime		= getSingleColumn('start_time',"select * from `event_times` where `date_id`='$eventStartDateId' ORDER BY `start_time` ASC LIMIT 0,1");

	//Convert to date
	$datestr	= $eventStartDate." ".$eventStartTime;
	...
	$timeTill = '';
		...
		if($days!=0)
			$timeTill .=$days.$d.", ".$hours."hrs";
		else
			$timeTill .=$hours."hrs, ". $min."mins";
...
function eventsRated($member_id){
	$res = mysql_query("select * from `events` where `userid`='$member_id'");
	while($row = mysql_fetch_array($res)){
		$ev_id = $row['id'];
		$res2 = mysql_query("select * from `comment` where `key_id`='$ev_id' && `c_type`='event' GROUP BY `key_id`");
		$a = number_format(mysql_num_rows($res2));
		$b = $a+$b;
	}
	if($b)
		return $b;
...
//function getTicketGross($ticket_id){
//	$prices = 0;
//	$res = mysql_query("select ot.order_id, ot.price from order_tickets ot, orders o where ot.ticket_id=$ticket_id && o.id=ot.order_id && o.total_price!=''");
//	while($row = mysql_fetch_array($res)){
//		
//		$order_id	= $row['order_id'];		
//		$discount	= getSingleColumn("discount","select * from `orders` where `id`='$order_id'");
//		
//		$price		= $row['price'];
//		
//		if($discount){
//			$ordrTotAmnt = getSingleColumn('tot',"select SUM(price) as tot from order_tickets where order_id=$order_id &&  `ticket_id`='$ticket_id'");		
//			$GrosPrc	= number_format(($price/$ordrTotAmnt)*(100),2);
//			$fGrosPrc	= number_format(($GrosPrc*$discount)/100,2);
//			$prices		= $prices+($price-$fGrosPrc);
//		}
//		else{
//			$prices		= $price + $prices;
//		}
...
function getTicketGross($id,$type){
	if($type == 'ticket'){
		$ticket_id	= $id;
		$prices		= 0;
		$res = mysql_query("select ot.order_id, ot.price from order_tickets ot, orders o where ot.ticket_id=$ticket_id && o.id=ot.order_id && o.total_price!=''");
	...
	elseif($type == 'event'){
		$gross_sale = 0;
		$event_id	= $id;
		$res2 = mysql_query("select * from `orders` where `total_price`!='' && `main_ticket_id`='$event_id' && `type`='ticket'");
		while($row2 = mysql_fetch_array($res2)){
			$order_id	= $row2['id'];
			$discount	= $row2['discount'];
			$discounts = $discount + $discounts;
			$res3 = mysql_query("select * from `order_tickets` where `order_id`='$order_id'");		
...
function getTicketGross2($id,$type){ // without calculate Discount 
	if($type == 'ticket'){
		$ticket_id	= $id;
		$prices		= 0;
		$res = mysql_query("select ot.order_id, ot.price from order_tickets ot, orders o where ot.ticket_id=$ticket_id && o.id=ot.order_id && o.total_price!=''");
...
function checkZipEvent($zip){
	$res = mysql_query("select * from `events` where `zipcode`='".$zip."' && `zipcode`!='0' && `zipcode`!='' && `is_expiring`='1' && `event_status`='1'");
		return mysql_num_rows($res);
}


function countSoldTickets($ticket_id){
	$res	= mysql_query("select * from `order_tickets` where `ticket_id`='$ticket_id'");
	while($row = mysql_fetch_array($res)){
	$qty		= $row['quantity'];
	$order_id	= $row['order_id'];
	$status	= getSingleColumn("id","select * from `orders` where `id`='$order_id' && `total_price`!='' && `total_price`!='0'");
	if($status > 0)
		$sold	= $qty+$sold;
	}
	if($sold > 0)
