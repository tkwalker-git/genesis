...
if($_GET["id"]){
if(if(validateID($_SESSION['LOGGEDIN_MEMBER_ID'],'tests',$_GET['id']) =='false'){
echo "<script>window.location.href='clinic_manager.php?p=test';</script>";
}
}

$bc_test_name	=	$_POST["test_name"];
$bc_description	=	$_POST["description"];
...
if (isset($_POST["submit"]) ) {
	...
	if (!count($errors)) {
		 ...
		 if ($action1 == "save") {
			$sql	=	"insert into tests (test_name,description,suppliers,cost,clinic_id) values ('" . $bc_test_name . "','" . $bc_description . "','" . $bc_suppliers . "','" . $bc_cost . "','" . $_SESSION['LOGGEDIN_MEMBER_ID'] . "')";
			$res	=	mysql_query($sql);
		...
		if ($action1 == "edit") {
			$sql	=	"update tests set test_name = '" . $bc_test_name . "', description = '" . $bc_description . "', suppliers = '" . $bc_suppliers . "', cost = '" . $bc_cost . "' where id=$frmID";
			$res	=	mysql_query($sql);
			if ($res) {
...
if ($res) {
	if ($row = mysql_fetch_assoc($res) ) {
		$bc_test_name	=	$row["test_name"];
		$bc_description	=	$row["description"];
...
<script language="javascript">
  ...
  <div id="middleContainer">
    <div class="creatAnEventMdl" style="font-size:55px; text-align:center; width:100%"> Create Test</div>
    ...
    <div class="gredBox">
      <form id="z_listing_event_form" action="" method="post" accept-charset="utf-8" onSubmit="return checkErr();" enctype="multipart/form-data" autocomplete="off">
            ...
            <div class="whiteMiddle" style="padding-top:1px;">
              <div id="accordion">
                ...
                <div id="box" class="box">
                  ...
                  <input type="text" name="cost" id="cost" class="new_input" value="<?php echo $bc_cost; ?>"/>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="create_event_submited">
            <input type="image" src="<?php echo IMAGE_PATH; ?>publish_new.png" name="submit" value="Create Test" align="right" />
