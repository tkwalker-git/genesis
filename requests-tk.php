...
-->
                            	<div class="yellow_bar">
									<table cellpadding="0"  cellspacing="0" width="99%" align="left">
										<tr>
											...
											<td width="58%">Patient Request Details</td>								
	...
	<?php
			...
			$sort	=	$_GET['sort'];
			$rest1 = "select * from `request_appt` where `clinicid`='". $_SESSION['LOGGEDIN_MEMBER_ID'] ."' order by status DESC";		
			}else {
...
<?php */?>
	...
	<td width="55%" valign="top" class="event_name">
	 	<?php  	$pla_id 	= getSingleColumn("id","select * from `plan` where `patient_id`='".$row['id']."' && clinic_id='".$member_id."' order by id desc limit 1");
				...
				$date	    = getSingleColumn("date_requested","select * from `request_appt` where `patient_id`='".$row['id']."' && clinic_id='".$_SESSION['LOGGEDIN_MEMBER_ID']."'");
				$rid	    = $row['id'];
				$time		= getSingleColumn("time_requested","select * from `request_appt` where `patient_id`='".$row['id']."' && clinic_id='".$_SESSION['LOGGEDIN_MEMBER_ID']."'");
				$reason		= getSingleColumn("reason","select * from `request_appt` where `patient_id`='".$row['id']."' && clinic_id='".$_SESSION['LOGGEDIN_MEMBER_ID']."'");	
				$pid		= $row['patient_id'];								
	...
	<table width="100%" border="0" cellspacing="7" cellpadding="0">
		<tr>
			<td width="30%">REQUEST DATE :</td><br />
			<td width="70%"><span><?php echo $date= date("M d, Y"); ?></span></td>
		</tr>
...
<!--     Insert Visit Info - End -->
  ...
  <tr>
	...
	$have=mysql_num_rows($check); ?>
							...
							<!--
<?php  if($pla_id){ ?><a href="view_plan_report.php?id=<?php echo $pla_id; ?>target="_blank"">Print Plan</a>	<br />	<br /><?php  } ?>
