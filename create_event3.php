...
//		$errors[] = 'Please enter Ticket Price';
	if ( trim($quantity_available)=='' && trim($mainTitle)!='' && $mainPrice!='')
      ...
      <form id="z_listing_event_form" action="" method="post" accept-charset="utf-8" onSubmit="return checkErr();" enctype="multipart/form-data" autocomplete="off">
		...
		?>
			...
			<?php
					...
					if($bc_event_status==1){ echo '<strong>Active</strong>'; }
        ...
        <td width="30%" class="z-time-cell z-col-3" style="background-color: #d1e5c0;">
			<div class="z-occurrence-start-time-layer" >
				...
				<select class="z-occurrence-start-am-pm" name="occurrences[<?php echo $unique_id ?>][start_am_pm]" >
					<option value="0" <?php if (date('A',strtotime($start_time[$i]))=='AM'){ echo "selected='selected'";}; ?>>AM</option>
					<option value="1" <?php if (date('A',strtotime($start_time[$i]))=='PM'){ echo "selected='selected'";}; ?>>PM</option>
				</select>
			</div>
...
if ($end_time!='00:00:00'){ echo date('h:i',strtotime($end_time)); }?>"  />
				<select class="z-occurrence-end-am-pm" name="occurrences[<?php echo $unique_id ?>][end_am_pm]"  >
					<option value="0" <?php if (date('A',strtotime($end_am_pm[$i]))=='AM'){ echo "selected='selected'";}; ?>>AM</option>
					<option value="1" <?php if (date('A',strtotime($end_am_pm[$i]))=='PM'){ echo "selected='selected'";}; ?>>PM</option>
				</select>
			</div>
		...
		value="<?php if ($start_sales_date!='0000-00-00' && $start_sales_date!=''){ echo date('d-M-Y', strtotime($start_sales_date)); } ?>" readonly="" id="start_sales_date" style="width:102px; color:#000000; cursor:pointer">
		   ...
		   ?>
                    ...
                    <div class="split_fee">
                      ...
                      Absorb the fees<br />
                      <br />
                      <input type="radio" name="split_fee" value="split" onChange="spltfee('s');" <?php if($event_id || isset($_POST["create"])){if($bc_buyer_service_fee!='0' && $bc_prometer_service_fee!='3'){ echo 'checked="checked"'; }} elseif($default_ticket_fees!=1 && $default_ticket_fees!=2 ){ echo 'checked="checked"'; } ?> />
                      Split the fees
                      <div class="clr"></div>
                      <br />
                      <br />
                      <div class="ev_fltlft" style="text-align:right"> Promoter Pays:
                        <input type="text" class="new_input" style="width:56px;" <?php if($event_id || isset($_POST["create"])){if($bc_buyer_service_fee=='0' || $bc_prometer_service_fee=='0'){ echo 'disabled="disabled"'; }} else{ if($default_ticket_fees!=3){ echo 'disabled="disabled"';}}?> name="prometer_service_free" id="prometer_service_free" 
onkeyup="updateTable('ajax/load_ticket_price_table.php');" onKeyPress="return isNumberKey(event)"  onblur="splitFee(this.value,'p'),updateTable();" value="<?php if($bc_prometer_service_fee || $bc_prometer_service_fee=='0'){ echo $bc_prometer_service_fee; }elseif($default_ticket_fees==1){ echo 0; }elseif($default_ticket_fees==2){ echo 100; }else{ echo '50';} ?>" />
                        ...
                        <br />
                        Customer Pays:
                        <input type="text" name="buyer_service_free" onKeyPress="return isNumberKey(event)" <?php if($event_id || isset($_POST["create"])){if($bc_buyer_service_fee=='0' || $bc_prometer_service_fee=='0'){ echo 'disabled="disabled"'; }}else{ if($default_ticket_fees!=2){ echo 'disabled="disabled"';}}?> id="buyer_service_free" value="<?php if($bc_buyer_service_fee || $bc_buyer_service_fee=='0'){ echo $bc_buyer_service_fee; }elseif($default_ticket_fees==0){ echo 100; }elseif($default_ticket_fees==1){ echo 0; }else{ echo '50';} ?>" onKeyUp="updateTable();"  onblur="splitFee(this.value,'c'),updateTable();" class="new_input" style="width:56px;" />
                      </div>
