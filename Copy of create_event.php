...
if($frmID == '' && $bc_event_type == '' && $_GET['type']!='premium')
	echo "<script>window.location.href='?type=premium'</script>";
...
if($bc_event_type==1 || $bc_event_type==2 || $bc_event_type==3){	
	
//	if ( trim($mainTitle) == '' )
//		$errors[] = 'Please enter Ticket Type';
//	if	( trim($mainPrice) == '' )
//		$errors[] = 'Please enter Ticket Price';
	if ( trim($mainQty)=='' && trim($mainTitle)!='' && $mainPrice!='')
		$errors[] = 'Please enter Quantity Available';
	if(	trim($bc_prometer_service_fee)=='' && trim($bc_buyer_service_fee)=='' && trim($mainTitle)!='' && $mainPrice!='')
		$errors[] = 'Please enter Split Percent';
	if(($bc_prometer_service_fee+$bc_buyer_service_fee)!=100 && trim($mainTitle)!='' && $mainPrice!='')
	...
	if (!count($errors)) {
		...
		$pym = attribValue("orders","total_price"," where main_ticket_id='$frmID' && `type`='flyer' && `total_price`!=''");
		if($bc_event_type!=0 && $pym==0){
		//	$event_status = "";
			$event_status = ", `event_status` = '$bc_event_status'";
		}
		else{
			$event_status = ", `event_status` = '$bc_event_status'";
			}
...
//	$sql = "UPDATE `events` SET `category_id` = '$bc_category_id', `subcategory_id` = '$bc_subcategory_id', `event_name` = '$bc_event_name',  $bc_seo_name `event_description` = '$bc_event_description', `event_cost` = '$bc_event_cost', $sql_img `event_age_suitab` = '$bc_event_age_suitab', `event_status` = '$bc_event_status', `added_by`='$bc_added_by',  `men_preferred_age` = '$bc_men_preferred_age', `women_preferred_age` = '$bc_women_preferred_age', `occupation_target` = '$bc_occupation_targets', `video_name` = '$bc_video_name', `video_embed` = '$bc_video_embed', `type` = '', `free_event`='$bc_free_event', `event_type`='$bc_event_type' WHERE `id` = '$frmID'";

	$sql = "UPDATE `events` SET `category_id` = '$bc_category_id', `subcategory_id` = '$bc_subcategory_id', `event_name` = '$bc_event_name',  $bc_seo_name `event_description` = '$bc_event_description', `event_cost` = '$bc_event_cost', $sql_img `event_age_suitab` = '$bc_event_age_suitab' $event_status ,  `added_by`='$bc_added_by',  `men_preferred_age` = '$bc_men_preferred_age', `women_preferred_age` = '$bc_women_preferred_age', `occupation_target` = '$bc_occupation_targets', `event_end_time` = '$bc_event_end_time', `type` = '', `free_event`='$bc_free_event', `event_type`='$bc_event_type', `alter_url`='$bc_alter_url', `zipcode`='$zipcode' WHERE `id` = '$frmID'";
 	...
 	$res = mysql_query($sql);
			...
			if($is_private){
				if($bc_private_emails){
					foreach($bc_private_emails as $private_emails){
						...
						$subject	= "EventGrabber - Event Invitation";
						@mail($private_emails,$subject,$contents,$headers);
					} // end for loop
				} // end if bc_private_emails

				if(isset($_FILES["private_emails_file"]) && !empty($_FILES["private_emails_file"]["tmp_name"]) && $_FILES["private_emails_file"]["type"] == 'text/plain'){
					$emails_file	= file_get_contents($_FILES['private_emails_file']['tmp_name']);
					$emails	= explode(",",$emails_file);
		...
		$bc_event_ticket_id	=	getSingleColumn('id',"select * from `event_ticket` where `event_id`='$frmID'");	
			...
			$mainDescription	= $_POST['mainDescription'];
				...
				$ticket_id = $t_id;
				if($mainTckId){
					mysql_query("UPDATE `event_ticket_price` set  `title`='$mainTitle', `price`='$mainPrice', `desc`='$mainDescription', `qty`='$mainQty' WHERE `id`='$mainTckId'");
					}
				else{
...
////////////////////////// //////
	...
	if($bc_event_type==1 || $bc_event_type==2 || $bc_event_type==3 || $_GET['r']=='py'){
		...
		$date		=	date('Y-m-d');
		$res = mysql_query("INSERT INTO `orders` (`id`, `user_id`, `total_price`, `discount`, `net_total`, `date`, `type`, `main_ticket_id`, `coupon_code`, `order_id`) VALUES (NULL, '$user_id', '1', '', '', '$date', 'premium', '$event_id', '', '$custom_order_id')");
...
if($frmID == ''){
	$res = mysql_query("select * from `event_default_settings` where `user_id`='$bc_userid'");
	while($row = mysql_fetch_array($res)){
	
		$bc_venu_id						= $row['venue'];
		$bc_category_id					= $row['category'];
		$bc_subcategory_id				= $row['sub_category'];
		$bc_added_by					= $row['host_name'];
		$bc_event_age_suitab			= $row['min_age'];
		$bc_men_preferred_age			= $row['men_age'];
		$bc_women_preferred_age			= $row['women_age'];
		$default_event_music			= $row['music_genres'];
		$quantity_available				= $row['tickets_quantity'];
		$default_cut_off_ticket			= $row['cut_off_ticket'];
		$default_ticket_sales_type		= $row['ticket_sales_type'];
		$default_ticket_fees			= $row['ticket_fees'];
		
		$bc_event_music			= explode(",",$default_event_music);
    ...
    <div class="gredBox">
      ...
      <div class="ev_new_box_center">
        <div style="position:relative; height:520px">
          <div class="basic_box">
            <div class="black">&nbsp;</div>
            <div class="detail">Add a basic listing of your event to our website for free.</div>
            <div align="center"><a href="javascript:voild(0)" onClick="window.location.href='create_event.php?type=simple';"><img src="images/ev_new_create_event.png" /></a><br />
               <a href="<?php echo ABSOLUTE_PATH; ?>plans-pricing.php" style="font-size:17px; color:#FFFFFF; margin-top:44px; display:block">Compare Options</a> 
            </div>
          </div>
          <!-- end basic_box -->
          <div class="featured_box">
            <div class="black">&nbsp;</div>
			<div class="detail">
				Featured Listings includes the followingupgrades:
				<ul>
					<li>Priority placement on the website and mobile platforms</li>
					<li>Box office (Sell tickets, promotional offers, packages or special deals)</li>
					<li>Share and connect with event attendees</li>
					<li>Large event image</li>
					<li>Photo gallery (Up to 15 photos)</li>
            ...
            <div align="center"><a  href="javascript:voild(0)" onClick="window.location.href='create_event.php?type=flyer';"><img src="images/ev_new_create_event.png" /></a><br />
               <a href="<?php echo ABSOLUTE_PATH; ?>plans-pricing.php" style="font-size:17px; color:#FFFFFF; margin-top:44px; display:block">Compare Options</a> 
            </div>
          </div>
          <!-- end featured_box -->
          <div class="premium_box">
            <div class="black">&nbsp;</div>
			<div class="detail">Premium Listings enhances Featured listings with following additions:
				<ul>
					<li>Highest priority placement on Eventgrabber and mobile platforms</li>
					<li>Photo gallery (unlimited images)</li>
					<li>Promotional videos (Up to 5 videos)</li>
					<li>Audience targeting</li>
					<li>Highlighted on Eventgrabber Email Campaigns</li>
					<li>Social media mentions</li>
					<li>No cost upfront, option to pay with proceeds</li>
				</ul>
			</div>
            <div align="center"> <a href="javascript:voild(0)"  onclick="window.location.href='create_event.php?type=premium';" ><img src="images/ev_new_create_event.png" /></a><br />
               <a href="<?php echo ABSOLUTE_PATH; ?>plans-pricing.php" style="font-size:17px; color:#FFFFFF; margin-top:44px; display:block">Compare Options</a> 
          ...
          <div class="custom_box">
            <div class="black">&nbsp;</div>
            <div class="detail">
				Showcase Creator allows you to take your Premium Listing anywhere:
				<ul>
					<li>Our proprietary interactive embedded event flyer</li>
					<li>Custom Facebook Fanpage app created</li>
				</ul>
			</div>
            <div align="center"><a  href="javascript:voild(0)" onClick="window.location.href='create_event.php?type=showcase'"><img src="images/ev_new_create_event.png" /></a><br />
               <a href="<?php echo ABSOLUTE_PATH; ?>plans-pricing.php" style="font-size:17px; color:#FFFFFF; margin-top:44px; display:block">Compare Options</a> 
            </div>
          </div>
          <!-- end custom_box -->
      ...
      <form id="z_listing_event_form" action="" method="post" accept-charset="utf-8" onSubmit="return checkErr();" enctype="multipart/form-data" autocomplete="off">
		...
		?>
			...
			<?php
					...
					if($bc_event_status==1 || $is_private){ echo '<strong>Active</strong>'; }
					elseif($bc_event_status==0){ echo '<strong>Inactive</strong>'; } 
				}
			}
		else{?>
  ...
  <td class="z-remove-cell z-col-4">
    <a class="z-occurrence-remove"><img src="images/icon_remove.gif" alt="remove" title="remove"></a>
                ...
                <div id="box" class="box" style="padding:30px 0">
				  ...
				  $resTicketDetails = mysql_query("select * from `event_ticket` where `event_id`='$event_id' && `event_id`!=''");
				  ...
				  $bc_buyer_service_fee		=	$rowTicketDetails['buyer_event_grabber_fee'];
				  $bc_prometer_service_fee	=	$rowTicketDetails['prometer_event_grabber_fee'];
				  }
				  ...
				  $mainQty				= $rowTicket['qty'];
				   ...
				   ?>
                      ...
                      <div class="evLabal" style="width:400px">
                        <input type="hidden" name="mainTckId" value="<?php echo $mainTckId; ?>" />
                      ...
                      <div class="clr"></div>
					  <div class="evField">Quantity Available:</div>
                      <div class="evLabal" style="width:300px">
                        <input type="text" class="new_input" name="mainQty" onKeyPress="return isNumberKey(event)" value="<?php echo $mainQty; ?>" style="width:60px;" onKeyUp="updateTable()">
                      </div>
                      <div class="clr"></div>
                      <div class="evField">Ticket Description:</div>
                      <div class="evLabal" style="width:300px;">
                        <textarea id="mainDescription" name="mainDescription" class="ticket_description"><?php if (isset($_POST["create"])){ echo $_POST['mainDescription']; } else{  echo $ticket_description; }?>
</textarea>
                      </div>
                      <div class="clr"></div>
                    </div>
                  ...
                  <?php
				   if ( is_array($_POST['ticketTypesOp']) ) {
						$bc_ticketType			= array();
						$bc_ticketPrice			= array();
						$bc_ticketIds			= array();
						$bc_ticketDescription	= array();
						$bc_ticketQty			= array();
						for($i=0;$i< count($_POST['ticketTypesOp']); $i++) {
							
							if($_POST['ticketTypesOp'][$i] == 'Other')
								$bc_ticketType[]	= $_POST['ticketTypes'][$i];
							else
								$bc_ticketType[]	= $_POST['ticketTypesOp'][$i];
								
								$bc_ticketPrice[]		= $_POST['ticketPrices'][$i];
								$bc_ticketIds[]			= $_POST['ticketIds'][$i];
								$bc_delT[]				= $_POST['delT'][$i];
								$bc_ticketDescription[]	= $_POST['ticketDescription'][$i];
								$bc_ticketQty[]			= $_POST['qty'][$i];
						  ...
						  while($rowTicket = mysql_fetch_array($resTicket)){
							  ...
							  if($i!=1){
								  $z++;						  
								  $bc_ticketType[]			= $rowTicket['title'];
								  $bc_ticketPrice[]			= $rowTicket['price'];
								  $bc_ticketIds[]			= $rowTicket['id'];
								  $bc_ticketDescription[]	= $rowTicket['desc'];
								  $bc_ticketQty[]			= $rowTicket['qty'];
							  }
						  }
					  } // end else
					  $z=0;
					  if ( is_array($bc_ticketType) ) {
						  for ( $i=0; $i < count($bc_ticketType); $i++ ){
							  $z++;
							  $ticketType		= $bc_ticketType[$i];
							  $ticketPrice		= $bc_ticketPrice[$i];
							  $ticketIds		= $bc_ticketIds[$i];
							  $ticketDescription= $bc_ticketDescription[$i];
							  $delT				= $bc_delT[$i];
							  $ticketQty		= $bc_ticketQty[$i];
							  
							  $ticketSold	= getSingleColumn("id","select * from `order_tickets` where `ticket_id`='$ticketIds'");
					   
					   ?>
					   
						  <div id="trow<?php echo $z; ?>"  <?php if ($delT){ echo 'style="display:none"'; }?> >
							<div style="border-bottom:1px solid #ccc;">
							  <div class="new_ticket_left">
								<div class="evField">Ticket Type: </div>
								<div class="evLabal" style="width:400px">
								  <input type="hidden" name="delT[]" value="<?php echo $delT; ?>"  id="del<?php echo $z; ?>" />
								...
								<div class="ev_fltrght" style="padding:10px; cursor:pointer"><img src="<?php echo IMAGE_PATH; ?>ticket_remove.png" <?php if ($ticketSold){?>onClick="alert('This ticket has been sold, So you cannot remove this ticket');"<?php }else{ ?> onClick="removeTicket(<?php echo $z; ?>);" <?php } ?> /></div>
								<div class="clr"></div>
								<div class="evField">Quantity Available:</div>
								  <div class="evLabal" style="width:300px">
									<input type="text" class="new_input" name="ticketQty[]" onKeyPress="return isNumberKey(event)" value="<?php echo $ticketQty; ?>" style="width:60px;" onKeyUp="updateTable()">
								  </div>
                    		  <div class="clr"></div>
								<div class="evField">Ticket Description:</div>
								<div class="evLabal" style="width:300px;">
								  <textarea id="" name="ticketDescription[]" class="ticket_description"><?php echo $ticketDescription; ?></textarea>
								</div>
								<div class="clr"></div>
							  </div>
							  <div class="new_ticket_right">
		...
		value="<?php if ($start_sales_date!='0000-00-00' && $start_sales_date!=''){ echo date('d-M-Y', strtotime($start_sales_date)); } ?>" readonly="" id="start_sales_date" style="width:102px; color:#000000; cursor:pointer">
		   ...
		   ?>
					  ...
					  <?php
					  if($bc_event_type=='3'){?>
                      <div class="evField">Alternative Buy Ticket URL:</div>
                      <div class="evLabal" style="width:300px">
					  	<input type="text" style="width:300px;" class="new_input" id="alter_url" name="alter_url" value="<?php echo $bc_alter_url; ?>" />
					  </div>
					  <div class="clr"></div>
					  <?php } ?>
                    </div>
                    <div class="split_fee">
                      <input type="radio" name="split_fee" value="prometer" onChange="spltfee('b');" <?php if($bc_prometer_service_fee==0 && $bc_buyer_service_fee!=0){ echo 'checked="checked"'; } elseif($default_ticket_fees == 1){ echo 'checked="checked"';} ?>  />
                      Pass on fees to buyer<br />
                      <br />
                      <input type="radio" name="split_fee" value="buyer" onChange="spltfee('a');" <?php if($bc_buyer_service_fee==0 && $bc_prometer_service_fee!=0){ echo 'checked="checked"'; } elseif($default_ticket_fees == 2){ echo 'checked="checked"';} ?>  />
                      Absorb the fees<br />
                      <br />
                      <input type="radio" name="split_fee" value="split" onChange="spltfee('s');" <?php if($event_id || isset($_POST["create"])){if($bc_buyer_service_fee!=0 && $bc_prometer_service_fee!=0){ echo 'checked="checked"'; }} elseif($default_ticket_fees!=1 && $default_ticket_fees!=2 ){ echo 'checked="checked"'; } ?> />
                      Split the fees
                      <div class="clr"></div>
                      <br />
                      <br />
                      <div class="ev_fltlft" style="text-align:right"> Promoter Pays:
                        <input type="text" class="new_input" style="width:56px;" <?php if($event_id || isset($_POST["create"])){if($bc_buyer_service_fee==0 || $bc_prometer_service_fee==0){ echo 'disabled="disabled"'; }}?> name="prometer_service_free" id="prometer_service_free" 
onkeyup="updateTable('ajax/load_ticket_price_table.php');" onKeyPress="return isNumberKey(event)"  onblur="splitFee(this.value,'p'),updateTable();" value="<?php if($bc_prometer_service_fee || $bc_prometer_service_fee=='0'){ echo $bc_prometer_service_fee; }elseif($default_ticket_fees==1){ echo 0; }elseif($default_ticket_fees==2){ echo 100; }else{ echo '50';} ?>" />
                        <br />
                        <br />
                        <br />
                        Customer Pays:
                        <input type="text" name="buyer_service_free" onKeyPress="return isNumberKey(event)" <?php if($event_id || isset($_POST["create"])){if($bc_buyer_service_fee==0 || $bc_prometer_service_fee==0){ echo 'disabled="disabled"'; }}?> id="buyer_service_free" value="<?php if($bc_buyer_service_fee || $bc_buyer_service_fee=='0'){ echo $bc_buyer_service_fee; }else{ echo '50';} ?>" onKeyUp="updateTable();"  onblur="splitFee(this.value,'c'),updateTable();" class="new_input" style="width:56px;" />
                      </div>
   ...
   ?>
		  ...
		  ?>
                  ...
                  <div  class="ev_fltlft" style="width:25%">
                    <div id="head" >Host Name</div>
                    <input type="text" class="new_input" name="added_by" value="<?php echo $bc_added_by; ?>" style="height: 20px; width: 208px;" />
                  </div>
                  ...
                  <div class="stpBox" style="float:right; width:450px">
                    ...
                    <div class="data" style="width:463px">
...
//						foreach($bc_gallery_images as $gallery_images){
                ...
                <div id="box" class="box">
                  <div id="head">Upload Members List :  <input type="file" name="private_emails_file" /> <i style="font-size:12px; color:#000000; font-weight:normal;">(Upload .txt file with comma separated)</i></div>
