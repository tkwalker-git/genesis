...
if (!$_SESSION['LOGGEDIN_MEMBER_ID']>0)
	...
	if($_GET["id"]){	
if (validateID($_SESSION['LOGGEDIN_MEMBER_ID'],'schedule_patient',$_GET["id"]) =='false')
		echo "<script>window.location.href='clinic_manager.php';</script>";
		}
		
if($_GET["pn"]){
$sql = "select * from `patients` where `id`='".$_GET['pn']."'";
	$r	=	mysql_query($sql);
...
$frmID	=	$_GET["id"];
//$action1 = isset($_POST["bc_form_action"]) ? $_POST["bc_form_action"] : "save";
...
if (isset($_POST["submit"]) ) {
	if (!count($errors)) {
	$have=mysql_query("select * from `schedule_patient` where `patient_id`='$bc_patient_id' && `clinic_id`='$bc_userid'");
		if(mysql_num_rows($have)){
		$action1="edit";
		}else {
		$action1="save";
		}
		...
		// save data here 
		 if ($action1 == "save") {
			 ...
			 $sql	=	"insert into schedule_patient (patient_id,clinic_id,comments,date) values ('" . $bc_patient_id . "','" . $bc_userid . "','" . $bc_event_description . "','" . $today . "')";			 
			$res	=	mysql_query($sql);
			
			$sql_status = mysql_query("insert into patients (status) values ('" . $patient_status . "')");
...
//////////////////////////////////
	$sql_date	=	mysql_query("insert into schedule_dates (patient_id,clinic_id,cons_date,start_time, end_time) values('" . $bc_patient_id . "','" . $bc_userid . "','" . $one_date . "','" . $startTime . "','" . $endTime . "')");
	
	$stats_pat	=	 getSingleColumn("status","select * from `patients` where `id`='$bc_patient_id'");
	if($stats_pat==1){
	mysql_query("update patients set status='2' where id='$bc_patient_id'");
	}
	...
	if ($sql_date) {
		...
		//end save here
	if ($action1 == "edit") {
			 $sql	=	"update schedule_patient set comments = '" . $bc_event_description . "' where `patient_id`='$frmID' && `clinic_id`='$bc_userid'";
			
			$res	=	mysql_query($sql);			
			
			mysql_query("delete from `schedule_dates` where `patient_id`='$frmID' && `clinic_id`='$bc_userid'");
...
$sql	=	"select * from schedule_patient where patient_id='$frmID' && clinic_id='$bc_userid'";
$res	=	mysql_query($sql);
if ($res) {
...
<script type="text/javascript" src="<?php echo ABSOLUTE_PATH; ?>js/yuiloader-min.js"></script>
<script type="text/javascript" src="<?php echo ABSOLUTE_PATH; ?>js/event-min.js"></script>
<script type="text/javascript" src="<?php echo ABSOLUTE_PATH; ?>js/dom-min.js"></script>
<script type="text/javascript" src="<?php echo ABSOLUTE_PATH; ?>js/calendar-min.js"></script>
<script type="text/javascript" src="<?php echo ABSOLUTE_PATH; ?>js/dragdrop-min.js"></script>
...
function add_more_image(id){
	...
	alert("You can not upload more than "+limitImage+" images");
	}
...
<div class="welcomeBox"></div>
  ...
  <div id="middleContainer">
    <div class="creatAnEventMdl" style="font-size:55px; text-align:center; width:100%"> Schedule Appointment</div>
        ...
        <div class="whiteTop">
          <div class="whiteBottom">
            <div class="whiteMiddle" style="padding-top:1px;">
              <div id="accordion">
                <h3>APPOINTMENT SCHEDULER</span></h3>
...
-->
                  ...
                  <br>
                  <a href="javascript:void(0)" style="color:#0066FF; text-decoration:underline" onClick="windowOpener(525,645,'Add New Patient','create_patient.php')"> New Patient? Click here to register </a>
                  <div class="clr"></div>
...
elseif($frmID){
				 $res = mysql_query("select * from `schedule_dates` where `patient_id`='$frmID' && `clinic_id`='$bc_userid' ORDER BY `cons_date` ASC");
				 $unique_id	=	0;
				 ...
				 ?>
                            <tr id="z_occurrence_row_<?php echo $unique_id ?>">
                              <td class="z-occurrence-date-cell z-col-1" style="background-color: #d1e5c0;"><input type="hidden" class="z-occurrence-id" name="occurrences[<?php echo $unique_id ?>][occurrence_id]" value="" />
                                <input type="hidden" class="z-occurrence-date" name="occurrences[<?php echo $unique_id ?>][date]" value="<?php echo date('m/d/Y', strtotime($row['cons_date'])); ?>" />
                                <?php echo date('D, d M, Y', strtotime($row['cons_date'])); ?> </td>
  ...
  <td class="z-remove-cell z-col-4">
    <a class="z-occurrence-remove"><img src="images/icon_remove.gif" alt="remove" title="remove"></a>
                ...
                <div id="head">Comments</div>
                  <div>
                    <textarea name="event_description" id="event_description" class="bc_input" style="width:868px; height:250px"><?php echo $bc_event_description; ?></textarea>
                  </div>
		...
		<input type="hidden" value="1" name="submit" />
			<a href="<?php echo ABSOLUTE_PATH; ?>dr_patient.php"><input type="image" src="<?php echo IMAGE_PATH; ?>submit-btn.png" name="submit" value="Create Event" style="cursor:pointer" onClick="save();"  align="right" /></a>
