...
function chng_pat_status(vala){
if(vala != ""){ 
$.post("set_checked_status.php", {centerid:vala},function(data) {
location.reload();   
    }
   ) }
}
...
strong{
    ...
    <?php
                                    ...
                                    $bg = "ffffff";
									$no=0;
                                    while($row = mysql_fetch_array($res)){ $no++;
										...
										$patient_stat   = getSingleColumn("status","select * from schedule_dates_status where cons_date='$today' && clinic_id='$member_id' && patient_id='$patient_id'");  
                                        $patient_name   = $patient_fname." ".   $patient_lname;
                                        $reason			= getSingleColumn("reason","select * from `request_appt` where `patient_id`='$patient_id' && clinic_id='$member_id'");
...
-->
            ...
            <tr>
                ...
                <td colspan="2" style="vertical-align:bottom;"><br /><br />
	<input type="radio" name="checked_id<?php echo $no; ?>" <?php if($patient_stat==2){?> checked="checked" <?php } ?> value="<?php echo $patient_id."_4"; ?>" onClick="chng_pat_status('<?php echo $row['id']."_2";  ?>');">&nbsp;Checked In&nbsp;&nbsp;&nbsp;
	
				<input type="radio" name="checked_id<?php echo $no; ?>" <?php if($patient_stat==1){?> checked="checked" <?php } ?> value="<?php echo $patient_id."_3";  ?>" onClick="chng_pat_status('<?php echo $row['id']."_1";  ?>');">&nbsp;No Show</td>
                
<!--
                <td height="7" colspan="2" align="right" style="padding-left:40px;"><?php 
