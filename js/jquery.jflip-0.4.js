...
;(function($){
    ...
    var Flip = function(canvas,width,height,images,opts) {
      ...
      gradientColors = opts.gradientColors || ['#EDBD7A','#FBF1E3','#fff'],
      curlSize = opts.curlSize || 0.2,
      scale = opts.scale,
      patterns = [],
...
// there //
 inCanvas = true;
      ...
      var
        ...
        flipDuration=1500,flipping = false,baseFlipX,baseFlipY,
        lastmX,lastmY,
        inCanvas = false,
        mousedown = false,
        dragging = false;
 ...
 //    alert("scrol");         //off = canvas.offset(); //update offset on scroll
...
// adil //			 sx = -1;
    ...
    $.fn.jFlip = function(width,height,opts){
      return this.each(function() {
        ...
        var canvas = $(document.createElement("canvas")).attr({width:width,height:height}).html('CLICK HERE').css({margin:0,width:width+"px",height:height+"px"})
