...
function getPages(abs_url,loadPage,responsId,event_id,extra)
{

	$.ajax({  
			type: "POST",
			url: abs_url+"ajax/"+loadPage,
			data:"event_id="+event_id+"&extra="+extra,
...
function getFlayer(abs_url,event_id,type)
{
	...
	else if(type=='fb' || type=='fbs' || type=='pangea'){
...
function showNextRecomeEvents2(abs_url,category_id,title,direction,pagenum,tot,list1)
{
	$.ajax({  
			type: "POST",  
			url: abs_url+"ajax/recomended_events_cat2.php",  
			data:"category_id="+category_id+"&title="+title+"&direction="+direction+"&page="+pagenum+"&tot="+tot+"&list="+list1,  
...
{	
	if(ty1=='comment')
		showOverlayer(abs_url + 'ajax/comment.php?event_id='+event_id1+'&type='+ty1,600);
	else
...
function postComment2(abs_url)
{
	...
	$.ajax({  
			...
			type: "POST",  
			url: abs_url+"ajax/submit_comment.php",  
			data:"id1="+id1+"&userid="+userid+"&ty="+ty+"&reviews="+reviews,  
			//dataType:"json",

			beforeSend: function()
			{
				if (reviews == ''){
					$("#errordiv").html('Please enter Comment.');
					return false;
				}
				else{
					$("#errordiv").html('Submitting Request...');
			...
			{  
				if ( resp == 1 ) {
					$("#errordiv").html('');
					$("#maincdiv").html('Comment is submitted successfuly.');
				} else
					$("#errordiv").html('There is some error. Please try again.');
			}, 
...
function showPages(abs_url,loadpage,err){
	var error =	encodeURI(err)
	if(error)
		var er = '?err='+error;
	else
		var er = '';
	showOverlayer(abs_url+loadpage+er,'rsvp');
	}
	
	
function rsvp(abs_url,event_id){
	showOverlayer(abs_url+'ajax/rsvp.php?event_id='+event_id,'rsvp');
	}
