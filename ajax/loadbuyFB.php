	...
	if(valid == 1){
		...
		if($('#discount_code').val()!=''){
			...
			var discount_code ='';
			 ...
			 $.ajax({  
				type: "POST",
				url: abs_url_secure+"ajax/loadbuy2FB.php",
...
?>
  <div class="ew-heading"><?php
	if(strlen(trim($event_name)) > 27)
		echo substr(trim($event_name),0,27)."...";
	else
		echo trim($event_name);
	  ?></div>
  <div style="padding:3px 0"><strong><?php echo date('M d,Y',strtotime($event_dateT[0]));?></strong></div>
  <strong><?php echo $venue_attrib[1]['venue_name']; ?></strong><br />
  <br />
  <table cellpadding="10" cellspacing="0" width="100%">
    ...
    <tr>
      <td colspan="5" style="padding:0"><div style="max-height: 266px; overflow: auto;">
          <?php
          ...
          <table cellpadding="10" cellspacing="0" width="100%">
            <tr bgcolor="<?php echo $bg; ?>">
              <td width="36%">
                ...
                <strong> <?php echo $row['title']; ?></strong> <small><?php echo substr($row['desc'],0,150); ?></small></td>
              <td width="13%"><strong>$<?php echo number_format($row['price'], 2,'.',''); ?></strong></td>
              <td width="16%"><strong>$
                <?php
	...
	?>
    <tr bgcolor="<?php echo $bg; ?>">
      <td colspan="5" align="right"><span class="addDiscountCode" onclick="$('#discount_code').css('display','block')">Add Discount Code</span> <span style="float:right">
        <input type="text" id="discount_code" style="display:none; width: 104px;" />
        </span></td>
    </tr>
    <?php
	if($bg == '#e4f0d8')
    ...
    <tr bgcolor="<?php echo $bg; ?>">
      <td><strong>Secure Payments By</strong>
        <iframe src="/secure_payment.php" width="150px" style="border:0; height:92px" scrolling="no"></iframe>
        <img src="<?php echo IMAGE_PATH; ?>cc_cards.png" />
        <!--<img src="<?php echo IMAGE_PATH; ?>secure_payment_paypal.png" />-->
      </td>
