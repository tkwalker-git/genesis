...
$disc	=  getSingleColumn("percent_value","select * from `coupons` where `code`='".$discount_code."'");
	...
	if($res){
		...
		$discount		= number_format(calculateDiscount($event_id,$discount_code,$total_prices),2);

		

		$net_total		= number_format($total_prices,2);

		$package_total	= number_format($total_prices - $discount,2);

		

		mysql_query("update `orders` set `discount`='$discount', `net_total`='$net_total' where `id`='$order_id'");
	...
	if($order_id){
		...
		$sql = "update `orders` set `total_price`='0.01' where `id`='$order_id'";
					...
					if($rs){
						...
						echo " <strong>An email has been sent to ".$email.", Check mail box for download tickets</strong>";
