...
if (isset($_POST["continue"]) ) {
	...
	if (!count($errors)) {
		 ...
		 if ($action1 == "save") {
			$sql	=	"insert into pt_inr (patient_id,add_date, add_time,reading,meal,eating_out,exercise,stress,fever,vomiting,notes) values ('" . $bc_patient_id . "','" . date('Y-m-d',strtotime($bc_add_date)) . "','". $bc_add_time ."','" . $bc_reading . "','" . $bc_meal . "','" . $eating_out . "','" . $exercise . "','" . $stress . "','" . $fever . "','" . $vomiting . "','" . $bc_notes . "')";
			$res	=	mysql_query($sql);
			 ...
			 $(function() {
             	$( "#dialog-confirm" ).dialog({
					  ...
					  buttons: {
						"Add More": function() {
						  $( this ).dialog( "close" );
						 window.location.href = 'pt_inr.php';
						},
						"I`m Done.": function() {
						  $( this ).dialog( "close" );
						  window.location.href = 'pt_inr.php?p=log';
						}
					  }
					});
			 });
		...
		if ($action1 == "edit") {
			$sql	=	"update pt_inr set patient_id = '" . $bc_patient_id . "', add_date = '" . date('Y-m-d',strtotime($bc_add_date)) . "', add_time = '" . $bc_add_time . "', reading = '" . $bc_reading . "', meal = '" . $bc_meal . "',eating_out = '".$eating_out."', exercise = '".$exercise."', stress ='".$stress."',fever = '".$fever."', vomiting = '".$vomiting."', notes = '" . $bc_notes . "' where id = '".$frmID."'";
			$res	=	mysql_query($sql);
			if ($res) {
			?>
           <script type="text/javascript">
...
$sql	=	"select * from pt_inr where id='".$frmID."'";
$res	=	mysql_query($sql);
if ($res) {
...
<div class="yellow_bar"> &nbsp; Add PT/INR Reading</div>
...
<?php } ?>
    ...
    <strong>PT/INR:</strong> Enter the PT/INR number displayed on your meter. It's important to be as accurate as you can with this entry, so try to enter it while your meter is nearby.<br /><br />
		<strong>Meal:</strong> Check the appropriate box if your PT/INR level was measured right before a meal or 1-2 hours after a meal. This could help you spot patterns so you can adjust your diet, as well as help you and your health care provider decide if you need to adjust your diabetes medication.<br /><br />
		<strong>PT/INR  Factors:</strong> Choose any icon that has occurred before or after this PT/INR reading. This could help you and your health care provider better manage your diabetes by understanding what affects your readings.<br /><br />

**To edit a PT/INR reading after leaving this page, click the reading on the View Graph tab or View Log tab.
	</div>
