...
class rssFeed{
	...
	function showDescription(){
		if($this->description)
			print "<p class='desc'>$this->description</p>\n";
	}
	
	function copyright(){
		if($this->copyright)
			print "<p class='copy'><b>$this->copyright</b></p>\n";
	}
...
//$new_Array = array();
//		
//		  for($i=count($this->stories)-1;$i>=0;$i--){
//		
//			$new_Array[]=$this->stories[$i];
//			
//		  }
//		foreach($new_Array as $story){
...
class newsStory{
	...
	function show(){
		if($this->title){
			if($this->link){
			$dt = date('D. M d', strtotime($this->pubdate));
				echo "<li><a target='_blank' href=\"$this->link\">$this->title</a> &nbsp;<span>$dt</span></li>\n";
			}elseif($this->title){
