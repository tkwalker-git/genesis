...
if($_GET["id"]){
	...
	$sql = "SELECT d.id , d.first_name , d.last_name , d.email , d.sex , c.id , c.user_id , c.clinicname ,c.address1 , c.city , c.state , c.zip , c.phone1 , c.fax1 , c.website , c.other_info FROM `doctors` AS `d` , clinics AS `c` WHERE  d.id = '". $_GET["id"] ."' AND c.user_id = '". $_GET["id"]. "'";
	$res = mysql_query($sql);
	
	$doctor = mysql_fetch_assoc($res);	
      ...
      <div class="whiteTop">
        <div class="whiteBottom">
          <div class="whiteMiddle" style="padding-top:7px;">
          	...
          	<div class="head_new">DOCTOR PROFILE</div>
            
            	<div class="recBox">
                    
                    <div style="padding:0 10px;"><br />
                    
					<div class="error" style="color:red; font-size:18px; text-align:center;"><?php if(isset($sucmess) and $sucmess == 1)echo 'Invitation send successfully'; ?></div>
                    ...
                    <div class="ew-heading">CLINIC INFORMATIONS </div>
                    
                    <table width="100%" border="0" cellspacing="10" cellpadding="0" style="border-top:1px solid #e5e5e5">
                      <tr>
                        <td width="150"><strong>Clinic Name:</strong></td>
                        <td><?php echo $doctor["clinicname"]; ?></td>
                      </tr>
                      <tr>
                      ...
                      <tr>
                        ...
                        <td><?php echo $doctor["State"]; ?></td>
                      </tr>
                      <tr>
                        <td><strong>Phone:</strong></td>
                        <td><?php echo $doctor["phone1"]; ?></td>
                      </tr>
                      <tr>
                        <td><strong>Fax:</strong></td>
                        <td><?php echo $doctor["fax1"]; ?></td>
                      </tr>
                      
                      <tr>
                        <td><strong>Other Information:</strong></td>
                        <td><?php echo $doctor["other_info"]; ?></td>
                      </tr>
                    </table>
...
$invitation_id = attribValue("invitations" , "COUNT(*)" , "WHERE `doctor_id` = '". $_GET["id"] ."' AND `patient_id` = '". $_SESSION["LOGGEDIN_MEMBER_ID"] ."'");

								if($invitation_id == 0){
							?>
                        <a href="<?php echo ABSOLUTE_PATH; ?>doctor_profile.php?id=<?php echo $doctor["id"]; ?>&invite=<?php echo $doctor["id"]; ?>"><strong>Click To Send Inviation</strong></a>
                        <?php }else{ echo "Inviations Already sent";} ?>
                        </td>
                      </tr>
