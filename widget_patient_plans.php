  ...
  <div class="blockerRepeat">
	...
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
        ...
        if(mysql_num_rows($reqw)){
			...
			$sql="select * from plan where patient_id=".$_SESSION['LOGGEDIN_MEMBER_ID']." && clinic_id='".$member_id."' ORDER BY plan_date DESC";
			$res=mysql_query($sql);
			if($res)
			{
				...
				<td class="botleftright"><a href="view_plan_report_user.php?id='<?php echo $row['id']; ?>'" target="_blank">View Plan</a></td>
				</tr>
				<?php }
			}
		?>
