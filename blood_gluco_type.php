...
if($_POST['save']=='Save'){
		...
		$page = $_POST['page'];
		if($check){
			mysql_query("update blood_gluco_setting set type='".$type."' where patient_id='".$bc_patient_id."' ") or die(mysql_error());
			echo '<script>
					window.location.href = "'.$page.'";
				</script>';
		}
		else{
			mysql_query("INSERT INTO blood_gluco_setting set type='".$type."' , patient_id='".$bc_patient_id."' ") or die(mysql_error());;
			echo '<script>
					window.location.href = "'.$page.'";
				</script>';
		}
...
<form  class="savemeta" action="blood_gluco_type.php" method="post">
<h1>Set Your Type</h1>
