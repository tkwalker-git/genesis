...
if(isset($_POST['cities']) && $_POST['cities'] != ""){
	...
	mysql_query("TRUNCATE TABLE events_temp");
	...
	for ( $a=1; $a<=5;$a++) {
		...
		for($j=0; $j<$locations_size; $j++){
			...
			*/
			...
			$event_query = "insert ignore into events_temp 
				(event_source,source_id,event_name,event_description,event_start_time,event_end_time,event_image,publishdate,added_by,tags,event_status) 
				VALUES ('EventFull','$event_id','$name','$descr','$start_time','$stop_time','$image','$addDate','$owner','$tgs','0')";
			...
			if( $event_insert_id > 0){
				...
				echo $venue_event_query = "insert into venue_events_temp (venue_id, event_id) values ('$venue_insert_id','$event_insert_id')" . "<br>";
			}
		}
