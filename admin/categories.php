...
if (isset($_POST["submit"]) ) {
	...
	if (!count($errors)) {
		 ...
		 if ($action1 == "save") {
			...
			$so 	= getSingleColumn('so',"select max(sort_order) as so from categories ");
			$so 	= ((int)$so) + 1 ;
			
			$sql	=	"insert into categories (name,seo_name,descr,sort_order,meta_title,meta_keywords,meta_description,tag) values ('" . $bc_name . "','" . $bc_seo_name . "','" . $bc_descr . "','" . $so . "','" . $bc_meta_title . "','" . $bc_meta_keywords . "','" . $bc_meta_description . "','". $bc_tags ."')";
			$res	=	mysql_query($sql);
		...
		if ($action1 == "edit") {
			$sql	=	"update categories set name = '" . $bc_name . "', descr = '" . $bc_descr . "', meta_title = '" . $bc_meta_title . "', meta_keywords = '" . $bc_meta_keywords . "', meta_description = '" . $bc_meta_description . "',tag='". $bc_tags ."' where id=$frmID";
			$res	=	mysql_query($sql);
			if ($res) {
...
$sql	=	"select * from categories where id=$frmID";
$res	=	mysql_query($sql);
if ($res) {
...
<tr>
<td align="right" class="bc_label">Category Name:</td>
...
<td align="left" class="bc_input_td">
<textarea  name="descr" class="bc_input" style="width:550px;height:250px;" /><?php echo $bc_descr; ?></textarea>
</td>
</tr>
...
<td align="left" class="bc_input_td">
<input type="text" name="meta_title" id="meta_title" style="width:550px" class="bc_input" value="<?php echo $bc_meta_title; ?>"/>
</td>
</tr>
