...
foreach ( $cats as $filter => $flt) {
	$unsigned_url2 = "http://api.yelp.com/v2/search?location=Orlando&category_filter=$filter";
	...
	while ( $row = mysql_fetch_assoc($res) ) {
		...
		$lng 	= $row['venue_lng'];
		$ven_id	= $row['id'];
		$sql1 = "select * from venues where substring(venue_lat,1,7)='". substr($lat,0,7) ."' AND substring(venue_lng,1,8)='". substr($lng,0,7) ."'
				ORDER BY CASE 
	...
	mysql_query("insert into venues select NULL,source_id,venue_type,venue_name,venue_address, 
