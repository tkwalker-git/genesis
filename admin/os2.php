...
function getGeoLocation2($name)
{
	// Address format jaranwala+road,+Faisalabad,+Punjab
	$url 	= "http://maps.googleapis.com/maps/api/geocode/xml?q=". urldecode($name) ."&address=Orlando,FL&sensor=true" ;
				...
				if($event_id){
					...
					////////////////// START VENUE WORK //////////////////
						...
						if($venue_id){
							...
							$venue_id	= getSingleColumn("id","select * from `venues` where `venue_lng`='$venue_lng' && `venue_lat`='$venue_lat'");
							if($venue_id){
								mysql_query("INSERT INTO `venue_events` (`id`, `venue_id`, `event_id`) VALUES (NULL, '$venue_id', '$event_id')");
							} // end if ($venue_id)
							else{
