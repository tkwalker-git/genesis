...
function insertSet($data, $table) {

		$separate="";
		$columns_values="";
		
		foreach ($data as $column => $value) {
			$columns_values.=$separate.' '.$column.'='."'".$value."'";
			$separate= ", ";
		}

		$sql = "insert into $table SET $columns_values";
...
function processRowSet($rowSet, $singleRow=false)
	{
...
function select($table, $where, $array=true) {
...
function delete($table, $where, $array=true) {
...
function getfield($table,$getfield,$idfield,$idvalue,$queryextra=NULL){
$query="select `$getfield` as fieldname from $table where $idfield='".$idvalue."' ".$queryextra;
$result=mysql_query($query) or error_log(mysql_error()._LINE_.$query.debug_print_backtrace());
if(mysql_num_rows($result)==0) return false;
$return=trim(mysql_result($result,0,'fieldname'));
