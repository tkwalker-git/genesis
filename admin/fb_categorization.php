...
while ($rows= mysql_fetch_assoc($res) ) {
	...
	$edesc		= strtolower(DBout($rows['event_description']));
...
if ( count($count) > 0 ) {
	foreach ($count as $e => $v) {
		...
		$sql5 = "update events set category_id='". $cat_id ."', subcategory_id='". $subc ."'". $age1 ." where id='". $e ."'";
		if ( mysql_query($sql5) )
			$tot++;
...
while ($rows= mysql_fetch_assoc($res) ) {
	...
	$ageS		= trim($rows['event_age_suitab']);
	...
	if ( separateTimes($rows['event_end_time']) == 'Night' ) {
		$category = 19;
		if ( $ageS == '' || $ageS == 0 )
			$sql5 = "update events set category_id='". $category ."', subcategory_id='80',event_age_suitab='". $age ."' where id='". $eid ."'";
		else
...
?>
<br>
<h1>Total Categorized Events: <?php echo $tot;?></h1>