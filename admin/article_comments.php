...
if (isset($_POST["submit"]) ) {
	...
	if (!count($errors)) {
		 ...
		 if ($action1 == "save") {
			$sql	=	"insert into article_comments (name,email,message,section,post_id,status,date_posted) values ('" . $bc_name . "','" . $bc_email . "','" . $bc_message . "','" . $bc_section . "','" . $bc_post_id . "','" . $bc_status . "','" . $bc_date_posted . "')";
			$res	=	mysql_query($sql);
		...
		if ($action1 == "edit") {
			$sql	=	"update article_comments set name = '" . $bc_name . "', email = '" . $bc_email . "', message = '" . $bc_message . "', section = '" . $bc_section . "', post_id = '" . $bc_post_id . "', status = '" . $bc_status . "', date_posted = '" . $bc_date_posted . "' where id=$frmID";
			$res	=	mysql_query($sql);
			if ($res) {
...
$sql	=	"select * from article_comments where id=$frmID";
$res	=	mysql_query($sql);
if ($res) {
...
<td align="left" class="bc_label">
<select name="post_id"  style="width:250px;" >
<?php	 echo getDropDown("articles", $bc_post_id,"title");	?>
</select>
