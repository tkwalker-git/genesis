...
strong{
    ...
    <?php
                                  ...
                                  /*?> include("page_form.php");
									...
									$qryf =mysql_query("select * from patients where clinicid ='". $_SESSION['LOGGEDIN_MEMBER_ID']."' && status='3'");
									while($alp = mysql_fetch_array($qryf)){
									$pa_id = $alp['id'];
									$qry2 = mysql_query("select * from patient_comments where comment_date='$today' && clinic_id='$member_id' && patient_id='".$pa_id."'  && status >= '0'");
									
									while($rec_dis = mysql_fetch_array($qry2)){
									
									$pid = $rec_dis['patient_id'];
									
                                    $sql = "select * from schedule_dates_status where cons_date='$today' && patient_id='$pid' && status >='0' && clinic_id='$member_id' order by start_time ";
