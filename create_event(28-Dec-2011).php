...
$sql = "UPDATE `events` SET `category_id` = '$bc_category_id', `subcategory_id` = '$bc_subcategory_id', `event_name` = '$bc_event_name',  $bc_seo_name `event_description` = '$bc_event_description', `event_cost` = '$bc_event_cost', $sql_img `event_age_suitab` = '$bc_event_age_suitab', `men_preferred_age` = '$bc_men_preferred_age', `women_preferred_age` = '$bc_women_preferred_age', `occupation_target` = '$bc_occupation_targets', `video_name` = '$bc_video_name', `video_embed` = '$bc_video_embed', `type` = '', `free_event`='$bc_free_event', `event_type`='$bc_event_type' WHERE `id` = '$frmID'";
	...
	mysql_query("INSERT INTO `event_ticket_price` (`id`, `title`, `price`, `ticket_id`) VALUES (NULL, '$mainTitle', '$mainPrice', '$ticket_id')");
		...
		if ( is_array($_POST['ticketTypes']) ) {
			for($i=0;$i< count($_POST['ticketTypes']); $i++) {
	...
	//			$event_ticket_price_id	=	$_POST['event_ticket_price_id'][$i];
		...
		} // END IF is_array($_POST['ticketTypes']
	} // END IF ($ticket_id)
...
///// ADD VIDEO START /////
		if($bc_video_embed){
			mysql_query("INSERT INTO `event_videos` (`id`, `video_name`, `video_embed`, `event_id`) VALUES (NULL, '$bc_video_name', '$bc_video_embed', '$event_id')");
		}
///// ADD VIDEO END /////
...
function add_anOtherTicket(id){
	var next_row = id+1;
	var new_url_feild = '<div id="trow'+next_row+'"><div class="new_ticket_left"><div class="evField">Ticket Type: </div><div class="evLabal" style="width:300px"><input type="text" style=" width:280px" onkeyup="updateTable();" value="" class="new_input" name="ticketTypes[]" id="title'+next_row+'" /></div> <div class="ev_fltrght" style="padding:10px; cursor:pointer"><img src="images/ticket_remove.png" onclick="removeTicket('+next_row+');" /></div> <div class="clr"></div><div class="evField">Price:</div><div class="evLabal" style="width:300px"><input type="text" class="new_input" name="ticketPrices[]" value="" id="costum_price'+next_row+'" onkeyup="updateTable(),extractNumber(this,2,true);" onkeypress="return blockNonNumbers(this, event, true, true);" style="width:60px;"></div><div class="clr"></div><div class="advanced_options" onclick="advancedOptions(\'s\');" id="advanceS" style="display:none;">Advanced Options <img src="images/arrow_dn.png" /></div><div class="clr"></div></div><div class="new_ticket_right"><table width="100%" border="0" cellspacing="0" cellpadding="10"><tr bgcolor="#e4f0d8"><td><strong>Ticket Type</strong></td><td align="right"><span id="t_type'+next_row+'"></span></td></tr><tr bgcolor="#d1e5c0"><td>Initial Price</td><td align="right"><span id="t_price'+next_row+'"></span></td></tr><tr bgcolor="#e4f0d8"><td>Promoter Fees</td><td align="right"><span id="t_pfees'+next_row+'"></span></td></tr><tr bgcolor="d1e5c0"><td>Customer Fees</td><td align="right"><span id="t_cfees'+next_row+'"></span></td> </tr> <tr bgcolor="e4f0d8"><td><strong>Final Price</strong></td><td align="right"><span class="t_finalPrice" id="t_finalPrice'+next_row+'"></span></td></tr></table></div></div>';
      ...
      <div class="ev_new_box_center">
        <div style="position:relative; height:520px">
          ...
          <div class="premium_box">
            ...
            <div align="center"><a href="javascript:voild(0)"  onclick="window.location.href='create_event.php?type=premium';"><img src="images/ev_new_create_event.png" /></a></div>
          </div>
          <!-- end premium_box -->
        ...
        <div class="whiteTop">
		  ...
		  ?>
                  <div class="ev_fltlft" style="width:25%">
                    <div id="head">Event Date</div>
                    <input type="text" class="new_input" name="event_date" readonly="" id="event_date" style="width:147px; color:#000000; cursor:pointer">
                  </div>
                  <div class="ev_fltlft" style="width:27%">
                    <div id="head">Start Time </div>
		  ...
		  if($i == '00'){
		   ...
		   ?>
                  ...
                  <div class="ev_fltlft" style="width:23%" align="right"><br>
                    <br>
                    <br>
                    <span id="addDateButton"><img src="<?php echo IMAGE_PATH; ?>add_date.png" id="addDate" onclick="add_date(<?php if($resDate){echo mysql_num_rows($resDate);}else{ echo 0;} ?>);" /></span> </div>
                  <div class="clr"></div>
				...
				?>
                    <div style="background:#D1E5C0;border-bottom: 1px solid #45BB96;font-size: 12px;font-weight: bold;line-height: 26px;padding: 10px;" id="row<?php echo $i; ?>">
                      <div class="ev_fltlft" style="width:32%"><?php echo date('d M Y', strtotime($row['event_date']));?>
					   <input type="hidden" name="occurrences[<?php echo $i; ?>][date]" value="<?php echo date('d M Y', strtotime($row['event_date']));?>"></div>
                      <div class="ev_fltlft" style="width:28%"><?php echo date('h:i A', strtotime($event_time['start_time'])); ?>
					  <input type="hidden" name="occurrences[<?php echo $i; ?>][start_time]" value="<?php echo date('h:i', strtotime($event_time['start_time'])); ?>">
					  <input type="hidden" name="occurrences[<?php echo $i; ?>][start_am_pm]" value="<?php echo date('A', strtotime($event_time['start_time'])); ?>"></div>
                      <div class="ev_fltlft" style="width:21%"><?php echo date('h:i A', strtotime($event_time['end_time'])); ?>
					  <input type="hidden" name="occurrences[<?php echo $i; ?>][end_time]" value="<?php echo date('h:i', strtotime($event_time['end_time'])); ?>">
					  <input type="hidden" name="occurrences[<?php echo $i; ?>][end_am_pm]" value="<?php echo date('A', strtotime($event_time['end_time'])); ?>"></div>
                      <div class="ev_fltlft" style="width:19%" align="center"><img src="<?php echo IMAGE_PATH; ?>closegreen.png" onclick="remove(<?php echo $i; ?>);" style="cursor:pointer" title="Remove this date"></div>
                      <div class="clr"></div>
                    </div>
                    <?php } } ?>
                  </div>
                ...
                <div id="box" class="box" style="padding:30px 0">
				  ...
				  $mainTicketType	= $rowTicket['title'];
				  $mainTicketPrice	= $rowTicket['price'];
				  }
				   ?>
                    <div class="evField">Ticket Type:<b class="clr">*</b> </div>
                    <div class="evLabal" style="width:300px">
                      <input type="text" style=" width:280px" onkeyup="updateTable();" value="<?php if ($_POST['create']){echo $_POST['mainTitle'];}else{ echo $mainTicketType; }?>" class="new_input" name="mainTitle" id="mainTitle" />
                    </div>
                    <div class="clr"></div>
                    <div class="evField">Price:<b class="clr">*</b></div>
                    <div class="evLabal" style="width:300px">
                      <input type="text" class="new_input" name="mainPrice" value="<?php if ($_POST['create']){echo $_POST['mainPrice'];}else{ echo $mainTicketPrice; }?>" id="mainPrice" onkeyup="updateTable(),extractNumber(this,2,true);" onkeypress="return blockNonNumbers(this, event, true, true);" style="width:60px;">
                    </div>
                    <div class="clr"></div>
...
onkeyup="updateTable('ajax/load_ticket_price_table.php');" onkeypress="return isNumberKey(event)"  onblur="splitFee(this.value,'p'),updateTable();" value="<?php if($prometer_ser_fee || $prometer_ser_fee=='0'){ echo $prometer_ser_fee; }else{ echo 50; } ?>" />
                  ...
                  <span id="add_tc"><img src="<?php echo IMAGE_PATH; ?>add_ticket.png" id="add_ticket" style="margin:10px 0 -20px 10px; cursor:pointer" onclick="add_anOtherTicket(<?php if($ticketNumRows){ echo $ticketNumRows-1;} else{ echo 0; }?>)" /></span> </div>
                <?php } ?>
                <h3>STEP
   ...
   ?>
		  ...
		  ?>
                  ...
                  <div class="stpBox" style="float:right; width:450px">
                    <div class="title" width:435px;">Music Details</div>
